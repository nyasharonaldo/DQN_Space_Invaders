...
class DeepQNetwork(nn.Module):
  def __init__(self, ALPHA):
    # Alpha is the learning rate for the stochastic gradient decent algo
    super(DeepQNetwork, self).__init__()
    self.conv1 = nn.Conv2d(1,32,8,stride=4,padding=1)
    self.conv2 = nn.Conv2d(32,64,4,stridde=2)
...
class Agent(object):
  def __init__(self, gamma, epsilon, alpha, maxMemorySize, epsEnd=0.05, replace=10000, actionSpace=[0,1,2,3,4,5]):
    ...
    alpha = learning rate
    maxMemorySize = Maxmimum memory size
    epsEnd = How low we want epsilon to go
    replace = How often we want to replace the target network
    
    Actions: 

    actionSpace = List of possible actions

    TODO: Look at Target Netowrk Replacement

    '''
    self.GAMMA = gamma
    self.EPSILON = epsilon
    self.EPS_END = epsEnd 
    self.actionSapce = actionSpace 
  ...
  def learn(self, batch_size):
    ...
    maxA = T.argmax(Qnext, dim=1).to(self.Q.eval.device)
